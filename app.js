...
const SAVE_KEY = 'terraforge_save_v4'; // マップ96化+深さ4+新鉱石/新機械/プレートでセーブ形式を更新

const ORES = {
  coal:    { name: '石炭',     depth: 1, color: 0x4b515f, ingotColor: 0x7c869c, oreValue: 3,  ingotValue: 10,  plateValue: 28,   amount: [45, 75] },
  iron:    { name: '鉄',       depth: 1, color: 0xb8bec9, ingotColor: 0xdfe5ee, oreValue: 4,  ingotValue: 15,  plateValue: 45,   amount: [35, 60] },
  copper:  { name: '銅',       depth: 2, color: 0xe08a3c, ingotColor: 0xffb060, oreValue: 8,  ingotValue: 32,  plateValue: 95,   amount: [30, 50] },
  silver:  { name: '銀',       depth: 2, color: 0xd4dce8, ingotColor: 0xf2f6fc, oreValue: 12, ingotValue: 48,  plateValue: 140,  amount: [25, 45] },
  gold:    { name: '金',       depth: 3, color: 0xffd23e, ingotColor: 0xffe680, oreValue: 20, ingotValue: 80,  plateValue: 240,  amount: [20, 35] },
  diamond: { name: 'ダイヤ',   depth: 3, color: 0x7de8ff, ingotColor: 0xc4f4ff, oreValue: 35, ingotValue: 140, plateValue: 420,  amount: [14, 26] },
  ruby:    { name: 'ルビー',   depth: 4, color: 0xe0335a, ingotColor: 0xff6b8f, oreValue: 55, ingotValue: 220, plateValue: 660,  amount: [10, 20] },
  mithril: { name: 'ミスリル', depth: 4, color: 0x4fe8b8, ingotColor: 0xb8ffe6, oreValue: 85, ingotValue: 340, plateValue: 1020, amount: [8, 16] },
};

const COSTS = { drill: 50, drill2: 220, conveyor: 10, fastConveyor: 25, smelter: 120, press: 250, seller: 80, splitter: 90, merger: 90, chest: 60, filterConveyor: 30 };
...
let tiles = [];               // [gx][gz] = {depth, ore:{type,amount}|null, mesh, oreMesh}
...
function createWorld(saved) {
    ...
    for (let gz = 0; gz < GRID; gz++) {
      ...
      for (const v of veins) {
        ...
        if (d <= v.r) {
          ...
          ore = { type: v.type, amount: spec.amount[0] + Math.floor(rand() * (spec.amount[1] - spec.amount[0])) };
          break;
        }
      }
  ...
  if (saved) {
    for (const t of saved.tiles) {
      ...
      if (!tile) continue;
      tile.depth = t.d;
      if (t.oa !== undefined && tile.ore) tile.ore.amount = t.oa;
...
function refreshTile(gx, gz) {
  ...
  const exposed = t.ore && ORES[t.ore.type].depth === t.depth && t.ore.amount > 0;
...
function buildFilterConveyorMeshShaped(shape) {
  const g = buildConveyorMeshShaped(shape);
  const gem = new THREE.Mesh(new THREE.OctahedronGeometry(0.16), new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0x333333, emissiveIntensity: .3, metalness: .4, roughness: .3 }));
...
const MESH_BUILDERS = { drill: buildDrillMesh, conveyor: () => buildConveyorMeshShaped('straight'), smelter: buildSmelterMesh, seller: buildSellerMesh, splitter: buildSplitterMesh, merger: buildMergerMesh, chest: buildChestMesh, filterConveyor: () => buildFilterConveyorMeshShaped('straight') };
...
function placeMachine(type, gx, gz, dir, silent) {
  ...
  const t = tiles[gx][gz];
  // ロード時(silent)は露出チェックをスキップ:枯れた鉱脈の上のドリルがリロードで消えるバグの修正
  ...
  if (!silent) {
    ...
    sfx('place');
    if (type === 'drill' && t.ore) toast('ドリルを設置!(' + ORES[t.ore.type].name + ' 残り' + t.ore.amount + '個)', 'good');
...
function toolLabel(t) {
  return { dig: '掘削', drill: 'ドリル', conveyor: 'コンベア', smelter: '精錬炉', seller: '販売機', demolish: '撤去', splitter: '分岐器', merger: '合流機', chest: 'チェスト', filterConveyor: 'フィルターコンベア' }[t] || t;
}
...
function outputDirsOf(m) {
  ...
  if (m.type === 'splitter') return [m.dir, (m.dir + 1) % 4, (m.dir + 3) % 4];
  return [m.dir];
}
...
function rebuildBeltMesh(m) {
  ...
  const shape = beltShapeFor(m);
  if (m.mesh && m.mesh.userData.beltShape === shape) {
    m.mesh.rotation.y = -m.dir * Math.PI / 2; // 向きだけ変わった場合の同期
    return;
  }
  const old = m.mesh;
  const nm = (m.type === 'conveyor' ? buildConveyorMeshShaped : buildFilterConveyorMeshShaped)(shape);
...
function canAccept(m, it, fromGx, fromGz) {
  ...
  if (m.type === 'filterConveyor') return !m.item && (m.filter === 'any' || m.filter === it.oreType);
...
function updateMachines(dt) {
  for (const m of machines.values()) {
    if (m.type === 'drill') {
      ...
      const hasOre = t.ore && ORES[t.ore.type].depth === t.depth && t.ore.amount > 0;
      if (hasOre) {
        ...
        if (m.timer >= DRILL_INTERVAL) {
          ...
          if (canAccept(target, dummy, m.gx, m.gz)) {
            ...
            if (t.ore.amount <= 0) { toast('鉱脈が枯れた…(' + ORES[t.ore.type].name + ')', 'error'); refreshTile(m.gx, m.gz); }
          }
        }
      ...
      if (m.item && !m.item.moving && m.item.gx === m.gx && m.item.gz === m.gz) {
        const nk = key(m.gx + DIRS[m.dir].x, m.gz + DIRS[m.dir].z);
...
function updateHighlight(clientX, clientY) {
  ...
  if (tool === 'dig') color = 0x6fd8ff;
    ...
    if (tool === 'drill') {
      const t = tiles[gx][gz];
      canPlace = canPlace && !!(t.ore && ORES[t.ore.type].depth === t.depth && t.ore.amount > 0); // ドリルは露出鉱石の上のみ
    }
    color = canPlace && money >= COSTS[tool] ? 0x7dffa0 : 0xff6b6b;
  }
  highlightMesh.material.color.setHex(color);
}
function hideHighlight() { highlightMesh.visible = false; }
  ...
  if (tool === 'dig') {
    if (existing) {
      ...
      if (existing.type === 'drill') {
        const hasOre = t.ore && t.ore.amount > 0;
        toast(hasOre ? '🛠️ ' + ORES[t.ore.type].name + 'を採掘中(残り' + t.ore.amount + '個)' : '🛠️ 鉱脈が枯れている…撤去しよう', hasOre ? 'good' : 'error');
        return;
      }
    ...
    if (exposed) {
      ...
      if (t.ore.amount <= 0) { toast(ORES[t.ore.type].name + '鉱脈を掘り尽くした', 'error'); }
      refreshTile(gx, gz);
      return;
    }
    ...
    refreshTile(gx, gz);
    const nowExposed = t.ore && ORES[t.ore.type].depth === t.depth && t.ore.amount > 0;
    if (nowExposed) {
      sfx('discover');
      toast('💎 ' + ORES[t.ore.type].name + '鉱脈を発見!(残り' + t.ore.amount + '個)ドリルを置こう!', 'good');
...
canvas.addEventListener('pointerup', e => {
  ...
  if (pointers.size === 0 && longPressFired) {
    longPressFired = false; // 長押しで向きを変えた場合はタップ操作を発火させない
...
function saveGame() {
  ...
  for (let gx = 0; gx < GRID; gx++) for (let gz = 0; gz < GRID; gz++) {
    const t = tiles[gx][gz];
    if (t.depth > 0 || (t.ore && t.ore.amount !== undefined)) {
      const rec = { x: gx, z: gz, d: t.depth };
      if (t.ore) rec.oa = t.ore.amount;
      tileData.push(rec);
    }
  }
